...
load("../stanfit_sirs/stanfit_sirs_shikoku_npi.rda")

ss <- summary(stanfit_sirs_shikoku_npi)

pop <- filter(japan_pop_island, island=="Shikoku")$pop
...
simulate_sirs_shikoku <- simlist %>%
  bind_rows()

save("simulate_sirs_shikoku", file="simulate_sirs_shikoku.rda")
